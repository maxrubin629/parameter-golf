...
class DepthMixer(nn.Module):
    ...
    # The generated experiments keep this lightweight: no per-source QKV matrices.
    def __init__(
        self,
        dim: int,
        max_sources: int,
        ...
        slots = max(num_loops, 1) if loop_specific else 1
        self.attnres_query = mx.zeros((slots, dim), dtype=mx.float32)
        self.attnres_bias = mx.zeros((slots, max_sources), dtype=mx.float32)
        if use_gate:
            self.attnres_gate = mx.zeros((slots, max_sources), dtype=mx.float32)
        self.scale = dim ** -0.5

    def __call__(self, sources: list[mx.array], loop_idx: int) -> mx.array:
        if not sources:
            raise ValueError("DepthMixer requires at least one source")
        if len(sources) > self.max_sources:
            raise ValueError(
                f"Full AttnRes source history has {len(sources)} sources, "
                f"but ATTNRES_MAX_SOURCES={self.max_sources}. Increase ATTNRES_MAX_SOURCES."
            )
        slot = loop_idx if self.loop_specific else 0
        stacked = mx.stack(sources, axis=2)  # [B, S, sources, D]
        nsrc = len(sources)
        query = self.attnres_query[slot].astype(stacked.dtype)
        keys = rms_norm(stacked)
        scores = mx.sum(keys * query[None, None, None, :], axis=-1) * self.scale
        scores = scores + self.attnres_bias[slot, :nsrc].astype(scores.dtype)[None, None, :]
        if self.use_gate:
            scores = scores + mx.tanh(self.attnres_gate[slot, :nsrc]).astype(scores.dtype)[None, None, :]
        weights = mx.softmax(scores.astype(mx.float32), axis=-1).astype(stacked.dtype)
        return mx.sum(weights[:, :, :, None] * stacked, axis=2)
...
class Block(nn.Module):
    ...
    def __call__(
        ...
        if self.attnres_enabled and attn_sources is not None:
            residual = x
            attn_in = self.attn_mixer(attn_sources, loop_idx)
        ...
        if self.attnres_enabled and mlp_sources is not None:
            mlp_in = self.mlp_mixer(mlp_sources, loop_idx)
        else:
            mlp_in = x
    ...
    def full_attnres_forward(
        ...
        # pseudo-query over the complete prior source history. The source list
        # starts with the token embedding, then appends the raw output of every
        # attention and MLP sublayer in unrolled depth order.
        attn_in = self.attn_mixer(history, loop_idx)
        attn_out = self.attn_scale.astype(attn_in.dtype)[None, None, :] * self.attn(self.attn_norm(attn_in))
        history.append(attn_out)

        mlp_in = self.mlp_mixer(history, loop_idx)
...
class GPT(nn.Module):
    ...
    def _source_window(self, sources: list[mx.array]) -> list[mx.array]:
        if len(sources) <= self.attnres_max_sources:
            return sources
        return sources[:1] + sources[-(self.attnres_max_sources - 1):]

    def _mean_source(self, sources: list[mx.array]) -> mx.array:
        if not sources:
            raise ValueError("_mean_source requires at least one source")
        acc = sources[0]
    ...
    def __call__(self, input_ids: mx.array) -> mx.array:
        ...
        # attention and MLP sublayer appends its raw output to this list, so later
        # sublayers can attend over every previous source plus the embedding.
        full_history: list[mx.array] = [x0]
        ...
        e = rms_norm(self._mean_source(full_history))
        h = e
        ...
        for loop_idx in range(self.num_loops):
            ...
            loop_hist_start = len(full_history)
            ...
            if self.attnres_prev_loop_blocks:
                ...
                full_history.append(self._mean_source(full_history[loop_hist_start:]))
