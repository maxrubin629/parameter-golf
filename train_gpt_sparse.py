...
class Hyperparameters:
    ...
    # grad_accum_steps=0 keeps the legacy auto behavior of max(1, 8 // WORLD_SIZE).
    ...
    save_final_artifacts = bool(int(os.environ.get("SAVE_FINAL_ARTIFACTS", "1")))
...
def validate_train_batch_geometry(args: Hyperparameters, world_size: int, grad_accum_steps: int) -> None:
    denom = world_size * grad_accum_steps
    if denom <= 0:
        raise ValueError(f"Invalid batch geometry denominator: world_size={world_size}, grad_accum_steps={grad_accum_steps}")
    if args.train_batch_tokens % denom != 0:
        raise ValueError(
            f"TRAIN_BATCH_TOKENS={args.train_batch_tokens} must be divisible by WORLD_SIZE*GRAD_ACCUM_STEPS={denom}"
        )
    local_tokens = args.train_batch_tokens // denom
    if local_tokens < args.train_seq_len:
        raise ValueError(
    ...
    if local_tokens % args.train_seq_len != 0:
        raise ValueError(
            "Local microbatch tokens must be divisible by TRAIN_SEQ_LEN; "
            f"got local_tokens={local_tokens}, TRAIN_SEQ_LEN={args.train_seq_len}"
        )
...
class MoEMLP(nn.Module):
    # Sparse router over SwiGLU experts. All `top_k>=1` routes use a sparse
    # pack -> batched expert matmul -> unpack path. This keeps compute
    # proportional to the selected expert assignments instead of materializing
    # every expert for every token. Capacity is fixed per batch from
    # capacity_factor * top_k * tokens / num_experts so the compiled shape stays static.
    def __init__(
        self,
        dim: int,
    ...
    def _compute_capacity(self, n_tokens: int) -> int:
        return max(1, math.ceil(self.capacity_factor * self.top_k * n_tokens / self.num_experts))

    def _forward_sparse(self, x: Tensor, router_logits: Tensor) -> tuple[Tensor, Tensor, Tensor]:
        bsz, seqlen, _ = x.shape
        n_tokens = bsz * seqlen
        x_flat = x.reshape(n_tokens, self.dim)

        router_logits_flat = router_logits.reshape(n_tokens, self.num_experts)
        router_probs = F.softmax(router_logits_flat, dim=-1)
        topk_idx = torch.topk(router_logits_flat, k=self.top_k, dim=-1, sorted=False).indices
        ...
        token_idx = (
            torch.arange(n_tokens, device=x.device, dtype=torch.int64)
            .unsqueeze(1)
            .expand(n_tokens, self.top_k)
            .reshape(-1)
        )
        assign_expert = topk_idx.reshape(-1)
        assign_prob = topk_probs.reshape(-1).to(dtype=x.dtype)

        sort_idx = torch.argsort(assign_expert, dim=0)
        sorted_token_idx = token_idx.index_select(0, sort_idx)
        sorted_experts = assign_expert.index_select(0, sort_idx)
        sorted_x = x_flat.index_select(0, sorted_token_idx)
        sorted_prob = assign_prob.index_select(0, sort_idx)

        n_assign = sort_idx.numel()
        expert_offsets = counts.cumsum(dim=0) - counts
        sorted_offsets = expert_offsets.index_select(0, sorted_experts)
        positions = torch.arange(n_assign, device=x.device, dtype=torch.int64) - sorted_offsets
        ...
        y_flat.index_add_(0, sorted_token_idx, gathered)
        return (
    ...
    def forward_with_aux(self, x: Tensor) -> tuple[Tensor, Tensor]:
        ...
        y, router_probs, hard_assign = self._forward_sparse(x, router_logits)
...
def main() -> None:
    ...
    validate_train_batch_geometry(args, world_size, grad_accum_steps)
    ...
    if expected_train_files is not None:
        log0(f"train_loader:expected_train_shards:{expected_train_files}")
    log0(f"val_loader:shards pattern={args.val_files} tokens:{val_tokens.numel() - 1} max_tokens:{args.val_max_tokens}")
    ...
    if distributed:
        ddp_model = DDP(
            ...
            bucket_cap_mb=(args.ddp_bucket_cap_mb if args.ddp_bucket_cap_mb > 0 else None),
            gradient_as_bucket_view=args.ddp_gradient_as_bucket_view,
            static_graph=args.ddp_static_graph,
        )
        model = torch.compile(ddp_model, dynamic=args.compile_dynamic) if args.enable_torch_compile else ddp_model
    else:
        model = torch.compile(base_model, dynamic=args.compile_dynamic) if args.enable_torch_compile else base_model
    ...
    log0(
        f"compile:enabled:{args.enable_torch_compile} dynamic:{args.compile_dynamic} "
        f"ddp_static_graph:{args.ddp_static_graph} ddp_gradient_as_bucket_view:{args.ddp_gradient_as_bucket_view} "
        f"ddp_bucket_cap_mb:{args.ddp_bucket_cap_mb}"
    )
    ...
    log0(
        f"save_final_artifacts:{args.save_final_artifacts} final_roundtrip_eval:{args.final_roundtrip_eval}"
    )
            ...
            for micro_step in range(grad_accum_steps):
                if distributed:
                    if ddp_model is None:
                        raise RuntimeError("ddp_model is required when distributed=True")
    ...
    while True:
        ...
        if should_log_train:
            tokens_per_s = (step * args.train_batch_tokens) / max(approx_training_time_ms / 1000.0, 1e-9)
            log0(
                f"step:{step}/{args.iterations} train_loss:{train_loss.item():.4f} "
                f"train_time:{approx_training_time_ms:.0f}ms step_avg:{approx_training_time_ms / step:.2f}ms "
                f"tok_s:{tokens_per_s:.1f} tok_s_per_gpu:{tokens_per_s / world_size:.1f}"
            )
    ...
    # -----------------------------
    # Save the raw state (useful for debugging/loading in PyTorch directly), then optionally
    # produce the compressed int8+zlib artifact and/or validate the round-tripped weights.
    quant_obj = None
    quant_raw_bytes = 0
    quant_blob = b""
    quant_stats = None
    if args.save_final_artifacts or args.final_roundtrip_eval:
    ...
    if args.save_final_artifacts and master_process:
        ...
        if quant_stats is None:
            raise RuntimeError("quant_stats is required when SAVE_FINAL_ARTIFACTS=1")
    ...
    if args.final_roundtrip_eval:
        if quant_obj is None:
            raise RuntimeError("quantized state is required when FINAL_ROUNDTRIP_EVAL=1")
        if distributed:
            dist.barrier()
