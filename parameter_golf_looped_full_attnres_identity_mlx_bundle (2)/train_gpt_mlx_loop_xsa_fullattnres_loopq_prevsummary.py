...
# ==============================================================================
# ARCHITECTURE VARIANT DEFAULTS
# ============================================================================== 
# These constants are overwritten per generated file. Most are still exposed as
# env vars so the same file can serve small sweeps without editing code.
ARCH_NAME = "loop_xsa_fullattnres_loopq_prevsummary"
ARCH_LOOPED = True
...
ARCH_ATTNRES_PREV_LOOP_BLOCKS = True
...
class Hyperparameters:
    ...
    # Looped decoder architecture controls. For the generated scripts these default
    # to the constants above, but env vars can sweep them without new files.
    arch_name: str = os.environ.get("ARCH_NAME", ARCH_NAME)
    ...
    attnres_max_sources: int = int(os.environ.get("ATTNRES_MAX_SOURCES", 128))
    # Identity-preserving Full AttnRes starts each mixer near "use the latest residual stream".
    # Larger values make older sources exponentially less likely at initialization.
    attnres_recency_bias_init: float = float(os.environ.get("ATTNRES_RECENCY_BIAS_INIT", 4.0))
    max_val_tokens: int = int(os.environ.get("MAX_VAL_TOKENS", 0))
    skip_quantized_validation: bool = bool(int(os.environ.get("SKIP_QUANTIZED_VALIDATION", "0")))
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,attnres_query,attnres_bias,attnres_gate,xsa_lambda,recur_decay,prelude_inject",
    ).split(",")
    if pattern
)
...
class CausalSelfAttention(nn.Module):
    ...
    # - RoPE on q and k
    # - causal masked SDPA
    # Optional XSA removes the component of the attention output along the current
    # token's own value vector. This is only enabled in recurrent core blocks by
    # the generated experiment files.
    def __init__(
        self,
        dim: int,
        num_heads: int,
        ...
        if xsa_enabled and xsa_learned:
            self.xsa_lambda = mx.ones((num_heads,), dtype=mx.float32) * xsa_lambda_init
        else:
    ...
    def __call__(self, x: mx.array) -> mx.array:
        ...
        if self.xsa_enabled:
            # Repeat GQA values to query-head count, then subtract the projection
            # of each head's attention output onto the token's own value vector.
            rep = self.num_heads // self.num_kv_heads
            v_self = mx.broadcast_to(
                v[:, :, None, :, :],
                (bsz, self.num_kv_heads, rep, seqlen, self.head_dim),
            ).reshape(bsz, self.num_heads, seqlen, self.head_dim)
            v_self = v_self.astype(y.dtype)
            v_self = v_self * mx.rsqrt(mx.sum(v_self * v_self, axis=-1, keepdims=True) + 1e-6)
            proj = mx.sum(y * v_self, axis=-1, keepdims=True) * v_self
            if self.xsa_learned:
                lam = self.xsa_lambda.astype(y.dtype)[None, :, None, None]
            else:
                lam = mx.array(self.xsa_lambda_value, dtype=y.dtype)
            y = y - lam * proj
...
class AttnResHistory:
    # Runtime-only Full AttnRes cache. The architecture is unchanged: every
    # residualized sublayer still attends over all previous source values.
    # The cache avoids re-normalizing the full history and avoids materializing
    # a [B, T, N, D] value stack inside every DepthMixer call.
    def __init__(self, first_source: mx.array, max_sources: int):
        self.max_sources = max_sources
        self.values: list[mx.array] = []
        self.keys: list[mx.array] = []
        self.total: mx.array | None = None
    ...
    def _check_capacity(self) -> None:
        if len(self.values) >= self.max_sources:
            raise ValueError(
                f"Full AttnRes source history would exceed ATTNRES_MAX_SOURCES={self.max_sources}. "
                "Increase ATTNRES_MAX_SOURCES."
            )

    def append(self, source: mx.array) -> None:
    ...
    def snapshot(self) -> tuple[int, mx.array]:
        if self.total is None:
            raise ValueError("AttnResHistory is empty")
        return len(self.values), self.total

    def mean_since(self, snapshot: tuple[int, mx.array]) -> mx.array:
        if self.total is None:
            raise ValueError("AttnResHistory is empty")
...
class DepthMixer(nn.Module):
    # Tiny Full-AttnRes mixer over same-position full prior sublayer-source states.
    # It uses a learned pseudo-query and source bias, optionally per loop slot.
    #
    # MLX optimization:
    # - source keys are RMS-normalized once at append time;
    # - score construction only stacks [B, T] score maps into [B, T, N];
    # - the value readout streams over source tensors instead of building a
    #   [B, T, N, D] value stack for every residualized sublayer.
    def __init__(
        self,
        dim: int,
        max_sources: int,
        ...
        self.attnres_query = mx.zeros((self.num_slots, dim), dtype=mx.float32)
        self.attnres_bias = mx.zeros((self.num_slots, max_sources), dtype=mx.float32)
        if use_gate:
            self.attnres_gate = mx.zeros((self.num_slots, max_sources), dtype=mx.float32)
        self.scale = dim ** -0.5

    def __call__(
        self,
        sources: AttnResHistory | list[mx.array],
        source_keys: list[mx.array] | None = None,
        loop_idx: int = 0,
    ) -> mx.array:
        if isinstance(sources, AttnResHistory):
            ...
            if source_keys is None:
                keys = [rms_norm(src) for src in values]
            else:
                keys = source_keys
        ...
        if nsrc > self.max_sources:
            raise ValueError(
                f"Full AttnRes source history has {nsrc} sources, "
                f"but ATTNRES_MAX_SOURCES={self.max_sources}. Increase ATTNRES_MAX_SOURCES."
            )
        if len(keys) != nsrc:
            raise ValueError(f"DepthMixer got {nsrc} source values but {len(keys)} source keys")
        if nsrc == 1:
            # Softmax over one source is exactly 1, regardless of bias/gate.
            return values[0]
        ...
        query = self.attnres_query[slot].astype(keys[0].dtype)
        q = query[None, None, :]
        scores = mx.stack(
            [mx.sum(key * q, axis=-1) * self.scale for key in keys],
            axis=-1,
        ).astype(mx.float32)
        if self.recency_bias_init != 0.0:
            recency = mx.arange(nsrc, dtype=mx.float32) - float(nsrc - 1)
            scores = scores + (self.recency_bias_init * recency)[None, None, :]
        scores = scores + self.attnres_bias[slot, :nsrc][None, None, :]
        if self.use_gate:
            scores = scores + mx.tanh(self.attnres_gate[slot, :nsrc])[None, None, :]

        weights = mx.softmax(scores, axis=-1).astype(values[0].dtype)
...
class Block(nn.Module):
    def __init__(
        ...
        attnres_recency_bias_init: float = 4.0,
    ):
        super().__init__()
        ...
        if attnres_enabled:
            self.attn_mixer = DepthMixer(dim, attnres_max_sources, attnres_num_loops, attnres_loop_specific, attnres_use_gate, recency_bias_init=attnres_recency_bias_init)
            self.mlp_mixer = DepthMixer(dim, attnres_max_sources, attnres_num_loops, attnres_loop_specific, attnres_use_gate, recency_bias_init=attnres_recency_bias_init)

    def __call__(
        self,
        x: mx.array,
        x0: mx.array,
        attn_sources: list[mx.array] | None = None,
        mlp_sources: list[mx.array] | None = None,
        loop_idx: int = 0,
    ) -> mx.array:
        if self.attnres_enabled and attn_sources is not None:
            ...
            x = residual + self.attn_scale.astype(x.dtype)[None, None, :] * attn_out
        else:
        ...
        if self.attnres_enabled and mlp_sources is not None:
            ...
            mlp_in = x
        x = x + self.mlp_scale.astype(x.dtype)[None, None, :] * self.mlp(self.mlp_norm(mlp_in))
        return x

    def full_attnres_forward(
        self,
        history: AttnResHistory,
        x0: mx.array,
        loop_idx: int = 0,
    ) -> mx.array:
        # Identity-preserving Full AttnRes mode. The normal residual stream stays
        # intact, while each sublayer can choose an alternate same-position prior
        # hidden state as its normalized input. We append post-residual hidden
        # states, not raw attention/MLP deltas, so the mixer starts from meaningful
        # residual-stream sources and can initialize near "latest source" identity.
        x = history.latest()
        attn_in = self.attn_mixer(history, loop_idx=loop_idx)
        attn_delta = self.attn_scale.astype(attn_in.dtype)[None, None, :] * self.attn(self.attn_norm(attn_in))
        x = x + attn_delta
        history.append(x)

        mlp_in = self.mlp_mixer(history, loop_idx=loop_idx)
        mlp_delta = self.mlp_scale.astype(mlp_in.dtype)[None, None, :] * self.mlp(self.mlp_norm(mlp_in))
        x = x + mlp_delta
        history.append(x)
        return x

class GPT(nn.Module):
    # Looped decoder-only GPT variant:
    # - token embedding + RMSNorm
    # - prelude blocks build an initial representation e
    # - recurrent core blocks are reused num_loops times
    # - coda blocks calibrate the final hidden state before tied LM head
    # - optional XSA lives only in recurrent core sequence attention
    # - optional Full-AttnRes mixers replace additive residual routing in every stage
    # - recurrent-core Full-AttnRes mixers get separate learned slots per loop iteration
    def __init__(
        self,
        vocab_size: int,
        num_layers: int,
        ...
        self.prelude_layers = args.prelude_layers if args.looped else num_layers // 2
        self.core_layers = args.core_layers if args.looped else 0
        self.coda_layers = args.coda_layers if args.looped else num_layers - self.prelude_layers
        ...
        total_blocks = self.prelude_layers + self.core_layers + self.coda_layers if args.looped else num_layers
        self.blocks = []
        for i in range(total_blocks):
            ...
            # Prelude/coda run once, so they use one mixer slot. Recurrent core blocks are reused,
            # so each loop iteration gets its own AttnRes query/bias/gate slot.
            ...
            self.blocks.append(
                Block(
                    ...
                    attnres_recency_bias_init=args.attnres_recency_bias_init,
                )
            )
        self.final_norm = RMSNormNoWeight()
        if self.attnres_enabled:
            self.final_mixer = DepthMixer(dim, args.attnres_max_sources, 1, False, False, recency_bias_init=args.attnres_recency_bias_init)

        if self.stable_recurrence:
            decay = min(max(args.recurrence_decay_init, 1e-4), 1.0 - 1e-4)
            decay_logit = math.log(decay / (1.0 - decay))
            self.recur_decay_logit = mx.ones((dim,), dtype=mx.float32) * decay_logit
            self.prelude_inject_scale = mx.ones((dim,), dtype=mx.float32) * args.recurrence_inject_scale
    ...
    def _stable_loop_input(self, h: mx.array, e: mx.array) -> mx.array:
        decay = 1.0 / (1.0 + mx.exp(-self.recur_decay_logit.astype(h.dtype)))
        inject = self.prelude_inject_scale.astype(h.dtype)
        return rms_norm(decay[None, None, :] * h + (1.0 - decay[None, None, :]) * inject[None, None, :] * e)
    ...
    def __call__(self, input_ids: mx.array) -> mx.array:
        ...
        if not self.looped:
            # Fallback keeps the base decoder shape without U-Net skips; generated
            # ablations use looped=True, while pg_baseline.py is the exact starter.
            for block in self.blocks:
                x = block(x, x0)
            return self.final_norm(x)

        if not self.attnres_enabled:
            # Additive-residual loop path, kept for env-var ablations.
            ...
            for loop_idx in range(self.num_loops):
                ...
                for j in range(self.core_layers):
                    h = self.blocks[core_start + j](h, e, loop_idx=loop_idx)
            coda_start = self.prelude_layers + self.core_layers
            for j in range(self.coda_layers):
                h = self.blocks[coda_start + j](h, e)
            return self.final_norm(h)

        # Identity-preserving Full Attention Residuals. The history is global
        # across the entire unrolled computation, but sources are post-residual
        # hidden states rather than raw sublayer deltas. This keeps the additive
        # residual stream as a safe default while exposing all prior hidden states
        # to each sublayer mixer.
        ...
        # Seed recurrence from the actual prelude residual stream, not an average
        # over all sources. This preserves the loop_stable inductive bias.
        e = rms_norm(h)
        h = e
        ...
        for loop_idx in range(self.num_loops):
            if self.stable_recurrence:
                h = self._stable_loop_input(h, e)
            # The loop boundary state is a lightweight synthetic source. It gives
            # the next unrolled recurrent step an explicit phase/state anchor,
            # while the actual transformer weights remain shared.
            ...
            if self.attnres_prev_loop_blocks:
                # Optional extra summary source. Full history is already present;
                # this ablates whether a direct prior-loop summary helps the tiny model.
...
class SplitOptimizers:
    ...
    # - non-control matrices: Muon
    # - small vectors/scalars/control matrices: Adam
    # This version is robust to looped models whose controls live outside blocks.*.
    ...
    def step(self, model: GPT, grads_tree: dict, step: int, lr_mul: float) -> None:
        ...
        scalar_params = {k: params[k] for k in scalar_grads}
        if scalar_grads:
...
def main() -> None:
    ...
    if args.max_val_tokens > 0:
        usable = (min(args.max_val_tokens, val_tokens.size - 1) // args.train_seq_len) * args.train_seq_len
        if usable <= 0:
            raise ValueError(f"MAX_VAL_TOKENS={args.max_val_tokens} is too small for TRAIN_SEQ_LEN={args.train_seq_len}")
    ...
    log(
        f"model_params:{n_params} arch:{args.arch_name} vocab_size:{args.vocab_size} "
        f"physical_layers:{len(model.blocks)} prelude:{args.prelude_layers} core:{args.core_layers} coda:{args.coda_layers} loops:{args.num_loops} "
        f"stable:{args.stable_recurrence} xsa:{args.xsa_enabled} xsa_learned:{args.xsa_learned} "
        f"attnres:{args.attnres_enabled} scope:identity_full_hidden_states recency_bias:{args.attnres_recency_bias_init:g} "
        f"attnres_core_loop_slots:{args.attnres_enabled} "
        f"prev_loop_blocks:{args.attnres_prev_loop_blocks} source_budget:{args.attnres_max_sources} "
    ...
    log(
        ...
        f"val_batch_size:{args.val_batch_size} max_val_tokens:{args.max_val_tokens} "
        f"warmup_steps:{args.warmup_steps} max_wallclock_seconds:{args.max_wallclock_seconds:.3f} "
        f"skip_quantized_validation:{args.skip_quantized_validation}"
    )
    ...
    if args.skip_quantized_validation:
        log("final_int8_zlib_roundtrip skipped:SKIP_QUANTIZED_VALIDATION=1")
        return
