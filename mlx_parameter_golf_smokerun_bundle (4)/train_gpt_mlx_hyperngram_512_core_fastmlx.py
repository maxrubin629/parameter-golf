...
class Hyperparameters:
    ...
    run_id: str = os.environ.get("RUN_ID", "hyperngram_512_core-" + str(uuid.uuid4()))
    ...
    num_layers: int = int(os.environ.get("NUM_LAYERS", 5))
    model_dim: int = int(os.environ.get("MODEL_DIM", 512))
    num_heads: int = int(os.environ.get("NUM_HEADS", 8))
    num_kv_heads: int = int(os.environ.get("NUM_KV_HEADS", 4))
    mlp_hidden_dim: int = int(os.environ.get("MLP_HIDDEN_DIM", 1536))
...
def rms_norm(x: mx.array, eps: float = 1e-6) -> mx.array:
    # Use MLX's fused RMSNorm kernel with no affine weight. It computes the same
    # last-axis normalization while avoiding the explicit square/mean/rsqrt graph.
...
class CausalSelfAttention(nn.Module):
    ...
    def __call__(self, x: mx.array) -> mx.array:
        ...
        q_gain = self.q_gain.astype(q.dtype).reshape(1, self.num_heads, 1, 1)
        q = q * q_gain
        y = mx.fast.scaled_dot_product_attention(q, k, v, scale=self.scale, mask="causal")
        if self.use_xsa:
            v_self = self._repeat_kv_to_heads(v, bsz, seqlen)
            if v_self.dtype != y.dtype:
...
class Block(nn.Module):
    ...
    def __call__(self, x: mx.array, x0: mx.array) -> mx.array:
        ...
        x = x + self.attn_scale.astype(x.dtype).reshape(1, 1, -1) * attn_out
        x = x + self.mlp_scale.astype(x.dtype).reshape(1, 1, -1) * self.mlp(self.mlp_norm(x))
        return x

class GPT(nn.Module):
    # HyperNgram-512 Core: wide 512d model with a prelude/core/coda loop schedule and a
    # low-rank neural n-gram residual before the tied LM head.
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int,
        ...
        self.arch_name = "hyperngram_512_core"
        self.dim = dim
        self.logit_chunk_tokens = logit_chunk_tokens
        self.logit_softcap = logit_softcap
        self.loop_scale_init = float(os.environ.get("LOOP_SCALE_INIT", "0.80"))
        self.ngram_mix_prev1 = float(os.environ.get("NGRAM_MIX_PREV1", "0.50"))
        self.ngram_mix_prev2 = float(os.environ.get("NGRAM_MIX_PREV2", "0.25"))

        self.tok_emb = nn.Embedding(vocab_size, dim)
        self.schedule = self._build_schedule(num_layers)
        self.num_virtual_layers = len(self.schedule)
        self.blocks = [
            Block(dim, num_heads, num_kv_heads, mlp_hidden_dim, rope_base, qk_gain_init, use_xsa=(1 <= i <= max(1, num_layers - 2)))
            for i in range(num_layers)
        ]
        self.loop_scales = mx.ones((self.num_virtual_layers, dim), dtype=mx.float32) * self.loop_scale_init
        self.ngram_proj = CastedLinear(dim, dim)
        self.ngram_scale = mx.ones((dim,), dtype=mx.float32) * float(os.environ.get("NGRAM_SCALE_INIT", "0.05"))
        self.final_norm = RMSNormNoWeight()

        for b in self.blocks:
            b.attn.proj.weight = mx.zeros_like(b.attn.proj.weight)
            b.mlp.proj.weight = mx.zeros_like(b.mlp.proj.weight)
        self.ngram_proj.weight = mx.zeros_like(self.ngram_proj.weight)
        self.tok_emb.weight = (
    ...
    def _build_schedule(self, num_layers: int) -> list[int]:
        ...
        if raw:
            ...
            return schedule
        if num_layers >= 5:
            cycles = max(1, int(os.environ.get("CORE_CYCLES", "4")))
            core = [1, 2, 3]
            return [0] + core * cycles + [num_layers - 1]
        core = list(range(1, max(1, num_layers - 1)))
        return [0] + core * 2 + [num_layers - 1]
    ...
    def _shift_right(self, x: mx.array, n: int) -> mx.array:
        ...
        pad = mx.zeros((x.shape[0], n, x.shape[2]), dtype=x.dtype)
        return mx.concatenate([pad, x[:, :-n, :]], axis=1)

    def _ngram_residual(self, emb: mx.array) -> mx.array:
        prev1 = self._shift_right(emb, 1)
        prev2 = self._shift_right(emb, 2)
        ctx = emb + self.ngram_mix_prev1 * prev1 + self.ngram_mix_prev2 * prev2
        res = self.ngram_proj(rms_norm(ctx))
        return self.ngram_scale.astype(res.dtype).reshape(1, 1, -1) * res
