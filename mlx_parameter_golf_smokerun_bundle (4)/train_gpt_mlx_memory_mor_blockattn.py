...
class GPT(nn.Module):
    ...
    def _depth_mix(self, x: mx.array, sources: list[mx.array], idx: int) -> mx.array:
        ...
        keys = rms_norm(vals)
        q = self.depth_queries[idx].astype(x.dtype)
        logits = mx.sum(keys * q[None, None, None, :], axis=-1) / math.sqrt(float(self.dim))
        weights = mx.softmax(logits.astype(mx.float32), axis=0).astype(x.dtype)
        mixed = mx.sum(weights[..., None] * vals, axis=0)
        gate = mx.tanh(self.depth_gates[idx]).astype(x.dtype)[None, None, :]
    ...
    def _adaptive_extra_loop(self, x: mx.array, x0: mx.array) -> mx.array:
        if self.extra_loop_passes <= 0 or not self.extra_schedule:
            return x
        route = mx.sigmoid((self.router(rms_norm(x)).astype(mx.float32) + self.route_bias.astype(mx.float32))).astype(x.dtype)
        for _ in range(self.extra_loop_passes):
            for j, block_idx in enumerate(self.extra_schedule):
                ...
                scale = self.extra_loop_scales[j].astype(x.dtype)[None, None, :]
