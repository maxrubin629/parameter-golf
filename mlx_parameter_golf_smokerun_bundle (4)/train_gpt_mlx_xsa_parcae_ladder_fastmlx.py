#!/usr/bin/env python3
"""
This is an experimental MLX smokerun fork of train_gpt_mlx.py for the xsa_parcae_ladder architecture. It is intended for local architecture testing, not as a polished SOTA submission script. We'll accept PRs that tune, improve, or simplify these scripts without significantly increasing complexity, but competitive submissions should stay in the `/records` folder.
...
class Hyperparameters:
    # Data / tokenizer. Defaults assume an SP8192 export; override these three vars for SP1024 smoke tests.
    ...
    run_id: str = os.environ.get("RUN_ID", "xsa_parcae_ladder-" + str(uuid.uuid4()))
    seed: int = int(os.environ.get("SEED", 1337))

    # Mac-first smoke defaults. Increase these for serious local comparisons.
    iterations: int = int(os.environ.get("ITERATIONS", 80))
    ...
    train_batch_tokens: int = int(os.environ.get("TRAIN_BATCH_TOKENS", 32_768))
    ...
    max_wallclock_seconds: float = float(os.environ.get("MAX_WALLCLOCK_SECONDS", 180.0))

    # Model. Parameter counts are <18M when VOCAB_SIZE=8192 and tied embeddings are enabled.
    ...
    mlp_hidden_dim: int = int(os.environ.get("MLP_HIDDEN_DIM", 1280))
    ...
    # Optimizer. Defaults are inherited from the MLX baseline style but clipped for safer architecture smoke tests.
...
def rms_norm(x: mx.array, eps: float = 1e-6) -> mx.array:
    # Fused RMSNorm avoids explicit square/mean/rsqrt temporaries and accumulates internally
    # in higher precision while preserving the caller-visible dtype.
...
class CausalSelfAttention(nn.Module):
    # GQA causal attention with optional Exclusive Self Attention (XSA).
    # XSA subtracts the component of the attention output in the direction of the current token's
    # own value vector, nudging attention capacity toward contextual information.
    def __init__(
        self,
        dim: int,
        num_heads: int,
    ...
    def _repeat_kv_to_heads(self, v: mx.array, bsz: int, seqlen: int) -> mx.array:
        if self.num_heads == self.num_kv_heads:
            return v
        repeat = self.num_heads // self.num_kv_heads
        v = mx.broadcast_to(
            v[:, :, None, :, :],
            (bsz, self.num_kv_heads, repeat, seqlen, self.head_dim),
        )
        return v.reshape(bsz, self.num_heads, seqlen, self.head_dim)
    ...
    def __call__(self, x: mx.array) -> mx.array:
        ...
        if self.use_xsa:
            # Keep the repeated-KV dimension implicit during the XSA projection instead of
            # materializing a full [B, H, T, D] copy of the value vectors.
            repeat = self.num_heads // self.num_kv_heads
            v_base = v.astype(y.dtype)
            v_norm = v_base * mx.rsqrt(mx.sum(v_base * v_base, axis=-1, keepdims=True) + 1e-6)
            y_grouped = y.reshape(bsz, self.num_kv_heads, repeat, seqlen, self.head_dim)
            v_norm = v_norm[:, :, None, :, :]
            y = (
                y_grouped
                - mx.sum(y_grouped * v_norm, axis=-1, keepdims=True) * v_norm
...
class MLP(nn.Module):
    # relu^2 MLP with explicit hidden size so the architecture can hit a tight parameter budget.
...
class Block(nn.Module):
    def __init__(
        ...
        self.resid_mix = mx.stack(
            (mx.ones((dim,), dtype=mx.float32), mx.zeros((dim,), dtype=mx.float32)),
            axis=0,
        )

    def __call__(self, x: mx.array, x0: mx.array) -> mx.array:
        dtype = x.dtype
        mix = self.resid_mix.astype(dtype)
        x = mix[0].reshape(1, 1, -1) * x + mix[1].reshape(1, 1, -1) * x0
        attn_out = self.attn(self.attn_norm(x))
        x = x + self.attn_scale.astype(dtype).reshape(1, 1, -1) * attn_out
        x = x + self.mlp_scale.astype(dtype).reshape(1, 1, -1) * self.mlp(self.mlp_norm(x))
        return x

class GPT(nn.Module):
    # XSA-Parcae Ladder: 10 physical blocks, 15 virtual applications, XSA in the
    # recurrent/later blocks, and a gated BlockAttnRes-lite depth mixer.
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int,
                 mlp_hidden_dim: int, logit_chunk_tokens: int, logit_softcap: float, rope_base: float,
                 tied_embed_init_std: float, qk_gain_init: float):
        super().__init__()
        if logit_softcap <= 0.0:
            raise ValueError(f"logit_softcap must be positive, got {logit_softcap}")
        self.arch_name = "xsa_parcae_ladder"
        self.dim = dim
        self.logit_chunk_tokens = logit_chunk_tokens
        self.logit_softcap = logit_softcap
        self.logit_inv_softcap = 1.0 / logit_softcap
        self.inv_sqrt_dim = 1.0 / math.sqrt(float(dim))
        ...
        self.loop_scale_init = float(os.environ.get("LOOP_SCALE_INIT", "0.85"))

        self.tok_emb = nn.Embedding(vocab_size, dim)
        self.schedule = self._build_schedule(num_layers)
        self.num_virtual_layers = len(self.schedule)
        xsa_start = int(os.environ.get("XSA_START_LAYER", "3"))
        self.blocks = [
            Block(dim, num_heads, num_kv_heads, mlp_hidden_dim, rope_base, qk_gain_init, use_xsa=(i >= xsa_start))
            for i in range(num_layers)
        ]
        # Starts as a no-op: gates are zero. Queries learn after gates open.
        self.depth_queries = mx.zeros((self.num_virtual_layers, dim), dtype=mx.float32)
        self.depth_gates = mx.zeros((self.num_virtual_layers, dim), dtype=mx.float32)
        self.loop_scales = mx.full((self.num_virtual_layers, dim), self.loop_scale_init, dtype=mx.float32)
    ...
    def _build_schedule(self, num_layers: int) -> list[int]:
        raw = os.environ.get("RECUR_SCHEDULE", "").strip()
        if raw:
            schedule = [int(x) for x in raw.replace(";", ",").split(",") if x.strip()]
            if not schedule or max(schedule) >= num_layers or min(schedule) < 0:
                raise ValueError(f"Invalid RECUR_SCHEDULE={raw!r} for NUM_LAYERS={num_layers}")
            return schedule
        if num_layers >= 10:
            return [0, 1, 2, 3, 4, 5, 3, 4, 5, 6, 7, 8, 5, 6, 9]
        core = list(range(1, max(1, num_layers - 1)))
        return [0] + core + core[: max(1, len(core) // 2)] + [num_layers - 1]

    def _depth_mix(self, x: mx.array, sources: list[mx.array], virtual_idx: int) -> mx.array:
        if not self.use_depth_mix or not sources:
            return x
        vals = mx.stack((*sources, x), axis=0)
        keys = rms_norm(vals)
        q = self.depth_queries[virtual_idx].astype(x.dtype).reshape(1, 1, 1, -1)
        logits = mx.sum(keys * q, axis=-1) * self.inv_sqrt_dim
        weights = mx.softmax(logits.astype(mx.float32), axis=0).astype(x.dtype)
        mixed = mx.sum(weights[..., None] * vals, axis=0)
        gate = mx.tanh(self.depth_gates[virtual_idx]).astype(x.dtype).reshape(1, 1, -1)
        return x + 0.5 * gate * (mixed - x)

    def softcap(self, logits: mx.array) -> mx.array:
        c = self.logit_softcap
        return c * mx.tanh(logits * self.logit_inv_softcap)

    def __call__(self, input_ids: mx.array) -> mx.array:
        x = rms_norm(self.tok_emb(input_ids).astype(COMPUTE_DTYPE))
        x0 = x
        sources: list[mx.array] = [x0]
        for virtual_idx, block_idx in enumerate(self.schedule):
            x = self._depth_mix(x, sources, virtual_idx)
            before = x
            block_out = self.blocks[block_idx](x, x0)
            scale = self.loop_scales[virtual_idx].astype(x.dtype)[None, None, :]
            x = before + scale * (block_out - before)
            if self.use_depth_mix and (virtual_idx + 1) % self.attnres_source_every == 0:
                sources.append(x)
    ...
    def loss(self, input_ids: mx.array, target_ids: mx.array) -> mx.array:
        ...
        tok_weight = self.tok_emb.weight.astype(x.dtype)
        if self.logit_chunk_tokens <= 0 or x.shape[0] <= self.logit_chunk_tokens:
            logits_proj = x @ tok_weight.T
        ...
        for s in range(0, n, self.logit_chunk_tokens):
            ...
            logits_proj = x[s:e] @ tok_weight.T
            logits = self.softcap(logits_proj)
...
class Muon:
    ...
    # parameter update.  This generalized version skips parameters that are intentionally unused
    # by a gated architecture path.
...
class SplitOptimizers:
    ...
    # - most 2D block/model matrices: Muon
    # - gates, depth mixers, and small control tensors: Adam
    # This keeps the baseline optimizer shape but supports architecture-specific modules.
    def __init__(self, model: GPT, args: Hyperparameters):
        self.args = args
        params = dict(tree_flatten(model.parameters()))
        self.embed_keys = [k for k in params if k == "tok_emb.weight"]
        self.matrix_keys = [
            k
    ...
    def step(self, model: GPT, grads_tree: dict, step: int, lr_mul: float) -> None:
        ...
        if embed_grads:
            embed_params = {k: params[k] for k in embed_grads}
...
def clip_grad_tree(grads_tree: dict, max_norm: float) -> dict:
    ...
    for grad in flat.values():
        ...
        total_sq = total_sq + mx.sum(grad32 * grad32)
    total_norm = mx.sqrt(total_sq)
    scale = mx.minimum(1.0, max_norm / (total_norm + 1e-12))
    return tree_unflatten([(k, (g * scale).astype(g.dtype)) for k, g in flat.items()])
...
def main() -> None:
    ...
    log(
        f"model_params:{n_params} arch:{getattr(model, 'arch_name', 'unknown')} vocab_size:{args.vocab_size} "
        f"layers:{args.num_layers} dim:{args.model_dim} heads:{args.num_heads} kv_heads:{args.num_kv_heads} "
        f"mlp_hidden_dim:{args.mlp_hidden_dim} seq_len:{args.train_seq_len} tie_embeddings:{args.tie_embeddings}"
    )
    log(
