#!/usr/bin/env python3
"""
This is an experimental MLX smokerun fork of train_gpt_mlx.py for the memory_mor_blockattn architecture. It is intended for local architecture testing, not as a polished SOTA submission script. We'll accept PRs that tune, improve, or simplify these scripts without significantly increasing complexity, but competitive submissions should stay in the `/records` folder.
...
class Hyperparameters:
    ...
    run_id: str = os.environ.get("RUN_ID", "memory_mor_blockattn-" + str(uuid.uuid4()))
    ...
    memory_slots: int = int(os.environ.get("MEMORY_SLOTS", 512))
...
class CausalSelfAttention(nn.Module):
    ...
    def __call__(self, x: mx.array) -> mx.array:
        ...
        q = q * self.q_gain.astype(q.dtype).reshape(1, self.num_heads, 1, 1)
        y = mx.fast.scaled_dot_product_attention(q, k, v, scale=self.scale, mask="causal")
        if self.use_xsa:
            v_self = self._repeat_kv_to_heads(v.astype(y.dtype), bsz, seqlen)
            v_norm = v_self * mx.rsqrt(mx.sum(v_self * v_self, axis=-1, keepdims=True) + 1e-6)
            y = y - mx.sum(y * v_norm, axis=-1, keepdims=True) * v_norm
...
class GPT(nn.Module):
    # Memory-MoR BlockAttn: 384d model with a small learned global memory,
    # gated block attention residuals, and token-wise adaptive extra passes.
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int,
        ...
        self.arch_name = "memory_mor_blockattn"
        self.dim = dim
        ...
        self.memory_slots = max(0, int(os.environ.get("MEMORY_SLOTS", "512")))
        self.extra_loop_passes = max(0, int(os.environ.get("EXTRA_LOOP_PASSES", "1")))

        self.tok_emb = nn.Embedding(vocab_size, dim)
        self.blocks = [
            Block(dim, num_heads, num_kv_heads, mlp_hidden_dim, rope_base, qk_gain_init, use_xsa=(i >= max(2, num_layers // 2)))
            for i in range(num_layers)
        ]
        self.depth_queries = mx.zeros((num_layers + 4, dim), dtype=mx.float32)
        self.depth_gates = mx.zeros((num_layers + 4, dim), dtype=mx.float32)
        self.mem_q = CastedLinear(dim, dim)
        if self.memory_slots > 0:
            self.memory_k = (mx.random.normal((self.memory_slots, dim), dtype=mx.float32) * 0.01).astype(COMPUTE_DTYPE)
            self.memory_v = (mx.random.normal((self.memory_slots, dim), dtype=mx.float32) * 0.01).astype(COMPUTE_DTYPE)
        else:
            self.memory_k = mx.zeros((1, dim), dtype=COMPUTE_DTYPE)
            self.memory_v = mx.zeros((1, dim), dtype=COMPUTE_DTYPE)
        self.memory_gate = mx.zeros((dim,), dtype=mx.float32)
        self.router = CastedLinear(dim, 1)
        self.route_bias = mx.array(np.array([-1.0], dtype=np.float32))
        self.extra_schedule = self._build_extra_schedule(num_layers)
        self.extra_loop_scales = mx.ones((max(1, len(self.extra_schedule)), dim), dtype=mx.float32) * 0.50
        self.final_norm = RMSNormNoWeight()

        for b in self.blocks:
            b.attn.proj.weight = mx.zeros_like(b.attn.proj.weight)
            b.mlp.proj.weight = mx.zeros_like(b.mlp.proj.weight)
        self.mem_q.weight = mx.zeros_like(self.mem_q.weight)
    ...
    def _depth_mix(self, x: mx.array, sources: list[mx.array], idx: int) -> mx.array:
        if not self.use_depth_mix or not sources:
            return x
        vals = mx.stack(sources + [x], axis=0)
        dtype = x.dtype
        keys = rms_norm(vals)
        q = self.depth_queries[idx].astype(dtype).reshape(1, 1, 1, self.dim)
        logits = mx.sum(keys * q, axis=-1) * self.inv_sqrt_dim
        weights = mx.softmax(logits.astype(mx.float32), axis=0).astype(dtype)
        mixed = mx.sum(weights[..., None] * vals, axis=0)
        gate = mx.tanh(self.depth_gates[idx]).astype(dtype).reshape(1, 1, self.dim)
        return x + 0.5 * gate * (mixed - x)

    def _memory_read(self, x: mx.array) -> mx.array:
        if self.memory_slots <= 0:
            return x
        q = rms_norm(self.mem_q(rms_norm(x))).astype(COMPUTE_DTYPE)
        k = rms_norm(self.memory_k.astype(q.dtype))
        logits = (q @ k.T) * self.inv_sqrt_dim
        weights = mx.softmax(logits.astype(mx.float32), axis=-1).astype(q.dtype)
        mem = weights @ self.memory_v.astype(q.dtype)
        gate = mx.tanh(self.memory_gate).astype(q.dtype).reshape(1, 1, self.dim)
        return x + 0.5 * gate * mem

    def _adaptive_extra_loop(self, x: mx.array, x0: mx.array) -> mx.array:
        if self.extra_loop_passes <= 0 or not self.extra_schedule:
            return x
        route_logits = self.router(rms_norm(x)).astype(mx.float32) + self.route_bias.astype(mx.float32)
        route = mx.sigmoid(route_logits).astype(x.dtype)
        for _ in range(self.extra_loop_passes):
            for j, block_idx in enumerate(self.extra_schedule):
                ...
                scale = self.extra_loop_scales[j].astype(x.dtype).reshape(1, 1, self.dim)
    ...
    def loss(self, input_ids: mx.array, target_ids: mx.array) -> mx.array:
        ...
        embed_weight_t = self.tok_emb.weight.astype(x.dtype).T
        if self.logit_chunk_tokens <= 0 or x.shape[0] <= self.logit_chunk_tokens:
            logits_proj = x @ embed_weight_t
        ...
        for s in range(0, n, self.logit_chunk_tokens):
            ...
            logits_proj = x[s:e] @ embed_weight_t
            logits = self.softcap(logits_proj)
