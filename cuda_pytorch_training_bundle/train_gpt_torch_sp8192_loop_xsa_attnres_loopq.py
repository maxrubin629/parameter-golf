#!/usr/bin/env python3
"""
Standalone CUDA/PyTorch port of the MLX architecture:
parameter_golf_looped_ablations_bundle (0)/train_gpt_mlx_sp8192_loop_xsa_attnres_loopq_fastmlx.py

The training runtime intentionally follows repo-root train_gpt.py: CUDA-only,
DDP over NCCL, bf16 autocast, torch.compile, Muon for matrix parameters, Adam
for embeddings/control tensors, fixed validation BPB reporting, and int8+zlib
...
ARCH_NAME = "sp8192_loop_xsa_attnres_loopq_fastmlx"
ARCH_LOOPED = True
...
class Hyperparameters:
    # Data/tokenizer defaults are the SP8192 bundle defaults from the MLX source.
    ...
    # Training length and batch shape. grad_accum_steps is derived in main() using
    # the train_gpt.py convention: 8 total accumulation slots split across ranks.
    ...
    attnres_max_sources = int(os.environ.get("ATTNRES_MAX_SOURCES", 6))
...
class DistributedTokenLoader:
    ...
    def next_batch(self, global_tokens: int, seq_len: int, grad_accum_steps: int) -> tuple[Tensor, Tensor]:
        ...
        if local_tokens < seq_len or local_tokens % seq_len != 0:
            raise ValueError(
                "TRAIN_BATCH_TOKENS must divide into whole local sequences; "
...
class CastedLinear(nn.Linear):
    # Keep weights in fp32 for optimizer/state quality, cast at matmul time.
    def forward(self, x: Tensor) -> Tensor:
...
def apply_rotary_emb(x: Tensor, cos: Tensor, sin: Tensor) -> Tensor:
    # Matches train_gpt.py and the MLX RoPE(..., traditional=False) convention.
    half = x.size(-1) // 2
    x1, x2 = x[..., :half], x[..., half:]
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.xsa_enabled:
            ...
            v_self = (
                v.to(dtype=y.dtype)[:, :, None, :, :]
                .expand(bsz, self.num_kv_heads, rep, seqlen, self.head_dim)
                .reshape(bsz, self.num_heads, seqlen, self.head_dim)
            )
            v_self = v_self * torch.rsqrt(torch.sum(v_self * v_self, dim=-1, keepdim=True) + 1e-6)
            proj = torch.sum(y * v_self, dim=-1, keepdim=True) * v_self
            if self.xsa_learned:
                lam = self.xsa_lambda.to(dtype=y.dtype)[None, :, None, None]
            else:
...
class DepthMixer(nn.Module):
    ...
    def __init__(
        ...
        self.attnres_query = nn.Parameter(torch.zeros(slots, dim, dtype=torch.float32))
        self.attnres_bias = nn.Parameter(torch.zeros(slots, max_sources, dtype=torch.float32))
        if use_gate:
            self.attnres_gate = nn.Parameter(torch.zeros(slots, max_sources, dtype=torch.float32))
        self.scale = dim**-0.5

    def forward(self, sources: list[Tensor], loop_idx: int) -> Tensor:
        if not sources:
        ...
        keys = F.rms_norm(stacked, (stacked.size(-1),))
        scores = torch.sum(keys * query[None, None, None, :], dim=-1) * self.scale
        scores = scores + self.attnres_bias[slot, :nsrc].to(dtype=scores.dtype)[None, None, :]
        if self.use_gate:
            scores = scores + torch.tanh(self.attnres_gate[slot, :nsrc]).to(dtype=scores.dtype)[None, None, :]
        weights = F.softmax(scores.float(), dim=-1).to(dtype=stacked.dtype)
...
class Block(nn.Module):
    def __init__(
        ...
        if attnres_enabled:
            self.attn_mixer = DepthMixer(
                dim,
                attnres_max_sources,
                attnres_num_loops,
                attnres_loop_specific,
                attnres_use_gate,
            )
            self.mlp_mixer = DepthMixer(
                dim,
                attnres_max_sources,
                attnres_num_loops,
                attnres_loop_specific,
...
class GPT(nn.Module):
    def __init__(
        ...
        if not tie_embeddings:
            raise NotImplementedError("This MLX architecture source only implements tied embeddings")
    ...
    def _stable_loop_input(self, h: Tensor, e: Tensor) -> Tensor:
        decay = torch.sigmoid(self.recur_decay_logit.to(dtype=h.dtype))
        inject = self.prelude_inject_scale.to(dtype=h.dtype)
        mixed = decay[None, None, :] * h + (1.0 - decay[None, None, :]) * inject[None, None, :] * e
        return F.rms_norm(mixed, (mixed.size(-1),))

    def forward_features(self, input_ids: Tensor) -> Tensor:
        x = self.tok_emb(input_ids)
        x = F.rms_norm(x.to(dtype=COMPUTE_DTYPE), (x.size(-1),))
        x0 = x
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        lm_weight = self.tok_emb.weight.to(dtype=x.dtype)
        if self.logit_chunk_tokens <= 0 or x.size(0) <= self.logit_chunk_tokens:
            logits = self.softcap(F.linear(x, lm_weight))
            return F.cross_entropy(logits.float(), targets, reduction="mean")

        # Chunked logits are retained as a memory knob. The default path is unchunked.
        ...
        for start in range(0, x.size(0), self.logit_chunk_tokens):
            ...
            logits = self.softcap(F.linear(x[start:end], lm_weight))
...
def main() -> None:
    ...
    if not args.tie_embeddings:
        raise NotImplementedError("This architecture source only supports tied embeddings")
    ...
    if len(embed_params) != 1:
        raise RuntimeError("Expected exactly one token embedding parameter")
    matrix_params = [
        p
    ...
    scalar_params = [
        p
        for name, p in named_params
        if name != "tok_emb.weight" and id(p) not in matrix_param_ids
    ]
    ...
    log0(f"model_name:{args.arch_name}")
    log0(
        f"model_params:{n_params} arch:{args.arch_name} vocab_size:{args.vocab_size} "
        f"physical_layers:{len(base_model.blocks)} prelude:{base_model.prelude_layers} "
        f"core:{base_model.core_layers} coda:{base_model.coda_layers} loops:{base_model.num_loops} "
        f"stable:{base_model.stable_recurrence} xsa:{args.xsa_enabled} xsa_learned:{args.xsa_learned} "
        f"attnres:{base_model.attnres_enabled} scope:all_blocks "
        f"attnres_core_loop_slots:{args.attnres_enabled} "
        f"prev_loop_blocks:{base_model.attnres_prev_loop_blocks} max_sources:{base_model.attnres_max_sources} "
    ...
    log0(
        ...
        f"embed_lr:{token_lr} matrix_lr:{args.matrix_lr} scalar_lr:{args.scalar_lr} "
        f"muon_momentum:{args.muon_momentum} muon_steps:{args.muon_backend_steps}"
    )
    log0(
        f"train_batch_tokens:{args.train_batch_tokens} train_seq_len:{args.train_seq_len} "
        f"iterations:{args.iterations} warmup_steps:{args.warmup_steps} "
        f"val_batch_size:{args.val_batch_size} max_wallclock_seconds:{args.max_wallclock_seconds:.3f}"
    )
    log0(f"compute_dtype:{COMPUTE_DTYPE} compile:True")
