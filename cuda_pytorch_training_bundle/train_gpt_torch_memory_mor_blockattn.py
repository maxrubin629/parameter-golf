#!/usr/bin/env python3
"""
Standalone CUDA/PyTorch port of the MLX memory_mor_blockattn_fastmlx architecture.

The architecture is sourced from:
  mlx_parameter_golf_smokerun_bundle (4)/train_gpt_mlx_memory_mor_blockattn_fastmlx.py

The training, validation, logging, distributed, and artifact conventions follow
the repo-root train_gpt.py style.
"""
...
MODEL_NAME = "memory_mor_blockattn"
SOURCE_VARIANT = "fastmlx"
...
class Hyperparameters:
    # Data / tokenizer defaults preserve the MLX architecture source. The train/val
    # glob expression follows the repo-standard train_gpt.py style.
    data_path = os.environ.get("DATA_PATH", "./data/datasets/fineweb10B_sp8192")
    train_files = os.path.join(data_path, "fineweb_train_*.bin")
    val_files = os.path.join(data_path, "fineweb_val_*.bin")
    tokenizer_path = os.environ.get("TOKENIZER_PATH", "./data/tokenizers/fineweb_8192_bpe.model")
    run_id = os.environ.get("RUN_ID", f"{MODEL_NAME}-" + str(uuid.uuid4()))
    seed = int(os.environ.get("SEED", 1337))

    # MLX smoke defaults, scaled across DDP ranks in the same way train_gpt.py
    # scales its 8-way accumulation budget.
    ...
    train_seq_len = int(os.environ.get("TRAIN_SEQ_LEN", os.environ.get("TRAIN_MAX_SEQ_LEN", 512)))
    ...
    mlp_hidden_dim = int(os.environ.get("MLP_HIDDEN_DIM", 1280))
    ...
    memory_slots = int(os.environ.get("MEMORY_SLOTS", 512))
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        (
            ...
            "q_gain,skip_weight,skip_weights,depth_query,depth_queries,depth_gate,"
            "depth_gates,loop_scale,loop_scales,extra_loop_scale,extra_loop_scales,"
            "ngram_scale,memory_gate,route_bias"
        ),
    ).split(",")
    if pattern
)
...
def zeropower_via_newtonschulz5(G: Tensor, steps: int = 5, eps: float = 1e-7) -> Tensor:
    # Follows train_gpt.py's distributed Muon path. The MLX source computes this
    # iteration in fp32; bf16 here is a deliberate CUDA throughput tradeoff.
...
class DistributedTokenLoader:
    def __init__(self, pattern: str, rank: int, world_size: int, device: torch.device, log_fn=None, dataset_name: str = ""):
        self.rank = rank
    ...
    def next_batch(self, global_tokens: int, seq_len: int, grad_accum_steps: int) -> tuple[Tensor, Tensor]:
        ...
        if local_tokens <= 0:
            raise ValueError(
                "TRAIN_BATCH_TOKENS is too small for this distributed shape; "
...
class CastedLinear(nn.Linear):
    # Keep weights in fp32 for optimizer state quality, matching the MLX source's
    # explicit fp32 CastedLinear weights.
    def forward(self, x: Tensor) -> Tensor:
...
class Rotary(nn.Module):
    # Uses the same half-rotate layout as train_gpt.py's PyTorch RoPE port. That is
    # the repo's existing approximation of MLX RoPE(traditional=False).
    def __init__(self, dim: int, base: float = 10000.0):
        super().__init__()
        self.dim = dim
        self.base = base
        inv_freq = self._make_inv_freq(device=torch.device("cpu"))
        self.register_buffer("inv_freq", inv_freq, persistent=False)
        self._seq_len_cached = 0
        self._cos_cached: Tensor | None = None
        self._sin_cached: Tensor | None = None

    def _make_inv_freq(self, device: torch.device) -> Tensor:
        return 1.0 / (self.base ** (torch.arange(0, self.dim, 2, dtype=torch.float32, device=device) / self.dim))
...
class CausalSelfAttention(nn.Module):
    ...
    def _repeat_kv_to_heads(self, v: Tensor) -> Tensor:
        if self.num_heads == self.num_kv_heads:
            return v
        repeat = self.num_heads // self.num_kv_heads
        return v.repeat_interleave(repeat, dim=1)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.use_xsa:
            v_self = self._repeat_kv_to_heads(v.to(dtype=y.dtype))
            v_norm = v_self * torch.rsqrt(torch.sum(v_self * v_self, dim=-1, keepdim=True) + 1e-6)
            y = y - torch.sum(y * v_norm, dim=-1, keepdim=True) * v_norm
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, x0: Tensor) -> Tensor:
        ...
        x = x + self.mlp_scale.to(dtype=dtype)[None, None, :] * self.mlp(self.mlp_norm(x))
        return x
...
class GPT(nn.Module):
    def __init__(
        ...
        self.dim = dim
        self.inv_sqrt_dim = dim ** -0.5
        self.logit_chunk_tokens = logit_chunk_tokens
        self.logit_softcap = logit_softcap
        self.use_depth_mix = bool(int(os.environ.get("USE_BLOCK_ATTNRES", "1")))
        self.attnres_source_every = max(1, int(os.environ.get("ATTNRES_SOURCE_EVERY", "2")))
        self.max_depth_sources = max(2, int(os.environ.get("MAX_DEPTH_SOURCES", "8")))
        self.memory_slots = max(0, int(memory_slots))
        self.extra_loop_passes = max(0, int(os.environ.get("EXTRA_LOOP_PASSES", "1")))
        ...
        self.blocks = nn.ModuleList(
            [
                Block(
                    ...
                    rope_base,
                    qk_gain_init,
                    use_xsa=(i >= max(2, num_layers // 2)),
                )
                for i in range(num_layers)
            ]
        )
        self.depth_queries = nn.Parameter(torch.zeros(num_layers + 4, dim, dtype=torch.float32))
        self.depth_gates = nn.Parameter(torch.zeros(num_layers + 4, dim, dtype=torch.float32))
        self.mem_q = CastedLinear(dim, dim, bias=False)
        mem_shape = (self.memory_slots, dim) if self.memory_slots > 0 else (1, dim)
        self.memory_k = nn.Parameter(torch.randn(mem_shape, dtype=torch.float32) * 0.01)
        self.memory_v = nn.Parameter(torch.randn(mem_shape, dtype=torch.float32) * 0.01)
        self.memory_gate = nn.Parameter(torch.zeros(dim, dtype=torch.float32))
        self.router = CastedLinear(dim, 1, bias=False)
        self.route_bias = nn.Parameter(torch.tensor([-1.0], dtype=torch.float32))
        self.extra_schedule = self._build_extra_schedule(num_layers)
        self.extra_loop_scales = nn.Parameter(torch.ones(max(1, len(self.extra_schedule)), dim, dtype=torch.float32) * 0.50)
        self.final_norm = RMSNormNoWeight()
        self.tied_embed_init_std = tied_embed_init_std
        self._init_weights()

    def _init_weights(self) -> None:
        # Non-zero linear initializers are left as PyTorch defaults to stay aligned
        # with train_gpt.py; the MLX default initializer may not be bit-identical.
        nn.init.normal_(self.tok_emb.weight, mean=0.0, std=self.tied_embed_init_std)
        for module in self.modules():
            if isinstance(module, nn.Linear) and getattr(module, "_zero_init", False):
                nn.init.zeros_(module.weight)
        nn.init.zeros_(self.mem_q.weight)
        nn.init.zeros_(self.router.weight)

    def _build_extra_schedule(self, num_layers: int) -> list[int]:
        raw = os.environ.get("EXTRA_LOOP_SCHEDULE", "").strip()
        if raw:
    ...
    def has_optional_unused_params(self) -> bool:
        return (
            not self.use_depth_mix
            or self.memory_slots <= 0
            or self.extra_loop_passes <= 0
            or not self.extra_schedule
        )
    ...
    def _depth_mix(self, x: Tensor, sources: list[Tensor], idx: int) -> Tensor:
        ...
        dtype = x.dtype
        keys = rms_norm(vals)
        q = self.depth_queries[idx].to(dtype=dtype).view(1, 1, 1, self.dim)
        logits = torch.sum(keys * q, dim=-1) * self.inv_sqrt_dim
        weights = F.softmax(logits.float(), dim=0).to(dtype=dtype)
        mixed = torch.sum(weights[..., None] * vals, dim=0)
        gate = torch.tanh(self.depth_gates[idx]).to(dtype=dtype).view(1, 1, self.dim)
    ...
    def _memory_read(self, x: Tensor) -> Tensor:
        ...
        gate = torch.tanh(self.memory_gate).to(dtype=q.dtype).view(1, 1, self.dim)
    ...
    def _adaptive_extra_loop(self, x: Tensor, x0: Tensor) -> Tensor:
        ...
        for _ in range(self.extra_loop_passes):
            for j, block_idx in enumerate(self.extra_schedule):
                ...
                scale = self.extra_loop_scales[j].to(dtype=x.dtype).view(1, 1, self.dim)
    ...
    def _loss_from_hidden(self, x: Tensor, target_ids: Tensor) -> Tensor:
        ...
        n = int(x.shape[0])
        for start in range(0, n, self.logit_chunk_tokens):
            end = min(start + self.logit_chunk_tokens, n)
            logits_proj = F.linear(x[start:end], self.tok_emb.weight.to(dtype=x.dtype))
            logits = self.logit_softcap * torch.tanh(logits_proj / self.logit_softcap)
            loss_sum = loss_sum + F.cross_entropy(logits.float(), targets[start:end], reduction="sum")
        return loss_sum / float(n)

    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        return self._loss_from_hidden(self.encode(input_ids), target_ids)
...
def main() -> None:
    ...
    if args.grad_accum_steps % world_size != 0:
        raise ValueError(
            ...
            "so each rank gets an integral accumulation count"
        )
    ...
    if master_process:
        ...
        logfile = os.path.join(args.out_dir, f"{args.run_id}.txt")
        print(logfile)
    ...
    if not args.tie_embeddings:
        raise NotImplementedError("memory_mor_blockattn preserves the MLX tied-embedding-only output head")
    ...
    for module in base_model.modules():
        ...
        if isinstance(module, Rotary):
            module.reset_cache(device)
    restore_low_dim_params_to_fp32(base_model)

    compiled_model: nn.Module = (
        torch.compile(base_model, dynamic=False, fullgraph=True) if args.torch_compile else base_model
    )
    model: nn.Module = (
        DDP(
            compiled_model,
            ...
            find_unused_parameters=base_model.has_optional_unused_params(),
        )
        if distributed
        else compiled_model
    )
    ...
    embed_params = [p for name, p in named_params if name == "tok_emb.weight"]
    matrix_params = [
        p
    ...
    if expected_train_files is None:
        ...
        log0(
            ...
            "new epochs will arrive sooner than the full dataset"
        )
    else:
        log0(f"train_loader:dataset:{dataset_name} train_shards:{actual_train_files}/{expected_train_files}")
    log0(
        f"model_params:{n_params} arch:{base_model.arch_name} source_variant:{base_model.source_variant} "
        f"vocab_size:{args.vocab_size} layers:{args.num_layers} dim:{args.model_dim} "
        f"heads:{args.num_heads} kv_heads:{args.num_kv_heads} mlp_hidden_dim:{args.mlp_hidden_dim} "
        f"seq_len:{args.train_seq_len} tie_embeddings:{args.tie_embeddings}"
    )
    log0(
        f"memory_slots:{base_model.memory_slots} use_block_attnres:{base_model.use_depth_mix} "
        f"attnres_source_every:{base_model.attnres_source_every} max_depth_sources:{base_model.max_depth_sources} "
        f"extra_loop_passes:{base_model.extra_loop_passes} extra_schedule:{','.join(map(str, base_model.extra_schedule))}"
    )
    log0(f"world_size:{world_size} grad_accum_steps:{grad_accum_steps} global_grad_accum_steps:{args.grad_accum_steps}")
    log0("sdp_backends:cudnn=False flash=True mem_efficient=False math=False")
    log0(f"attention_mode:gqa+xsa num_heads:{args.num_heads} num_kv_heads:{args.num_kv_heads}")
    log0(
    ...
    log0(f"compute_dtype:{COMPUTE_DTYPE} torch_compile:{args.torch_compile}")
