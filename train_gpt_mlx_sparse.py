...
# HYPERPARAMETERS
# ==============================================================================
# Default sparse recurrent AttnRes run:
...
class Hyperparameters:
    ...
    # Training loop. These defaults mirror train_gpt_sparse.py where that makes sense for a single-process MLX run.
    iterations: int = int(os.environ.get("ITERATIONS", 100_000))
    ...
    # Model (defaults match the sparse recurrent AttnRes starting point).
    ...
    mlp_kind: str = os.environ.get("MLP_KIND", "moe").strip().lower()
    moe_num_experts: int = int(os.environ.get("MOE_NUM_EXPERTS", 16))
    moe_top_k: int = int(os.environ.get("MOE_TOP_K", 2))
    moe_expert_mlp_mult: float = float(os.environ.get("MOE_EXPERT_MLP_MULT", 0.125))
    ...
    # Optimizer defaults match train_gpt_sparse.py for the tied-embedding MLX split.
    beta1: float = float(os.environ.get("BETA1", 0.9))
    beta2: float = float(os.environ.get("BETA2", 0.95))
    adam_eps: float = float(os.environ.get("ADAM_EPS", 1e-8))
    tied_embed_lr: float = float(os.environ.get("TIED_EMBED_LR", 0.0075))
    matrix_lr: float = float(os.environ.get("MATRIX_LR", 0.006))
    scalar_lr: float = float(os.environ.get("SCALAR_LR", 0.006))
...
class MoEMLP(nn.Module):
    ...
    def _compute_capacity(self, n_tokens: int) -> int:
        return max(1, int(math.ceil(self.capacity_factor * self.top_k * n_tokens / self.num_experts)))

    def _forward_sparse(self, x: mx.array, router_logits: mx.array) -> tuple[mx.array, mx.array, mx.array]:
        bsz, seqlen, _ = x.shape
        n_tokens = bsz * seqlen
        n_assign = n_tokens * self.top_k
        x_flat = x.reshape(n_tokens, self.dim)

        router_logits_flat = router_logits.reshape(n_tokens, self.num_experts)
        router_probs = mx.softmax(router_logits_flat, axis=-1)
        topk_idx = mx.stop_gradient(mx.argsort(router_logits_flat, axis=-1)[..., -self.top_k :].astype(mx.int32))
        topk_probs = mx.take_along_axis(router_probs, topk_idx, axis=-1)

        hard_assign = mx.put_along_axis(
            mx.zeros_like(router_probs),
            topk_idx,
            mx.ones_like(topk_probs),
            axis=-1,
        )
        counts = mx.sum(hard_assign, axis=0).astype(mx.int32)

        token_idx = mx.arange(n_tokens, dtype=mx.int32).reshape(n_tokens, 1)
        token_idx = mx.broadcast_to(token_idx, (n_tokens, self.top_k)).reshape(n_assign)
        assign_expert = topk_idx.reshape(n_assign)
        assign_prob = topk_probs.reshape(n_assign).astype(x.dtype)

        sort_idx = mx.stop_gradient(mx.argsort(assign_expert, axis=0).astype(mx.int32))
        sorted_token_idx = mx.take(token_idx, sort_idx, axis=0)
        sorted_experts = mx.take(assign_expert, sort_idx, axis=0)
        sorted_x = mx.take(x_flat, sorted_token_idx, axis=0)
        sorted_prob = mx.take(assign_prob, sort_idx, axis=0)

        expert_offsets = mx.cumsum(counts, axis=0) - counts
        sorted_offsets = mx.take(expert_offsets, sorted_experts, axis=0)
        positions = mx.arange(n_assign, dtype=mx.int32) - sorted_offsets
        ...
        packed_inputs = mx.zeros((self.num_experts * (capacity + 1), self.dim), dtype=x.dtype)
        packed_inputs = packed_inputs.at[flat_slot].add(sorted_x * keep.astype(sorted_x.dtype)[:, None])
        packed_inputs = packed_inputs.reshape(self.num_experts, capacity + 1, self.dim)
        ...
        gathered = mx.take(
            packed_outputs.reshape(self.num_experts * (capacity + 1), self.dim),
            flat_slot,
            axis=0,
        )
        gathered = gathered * (sorted_prob * keep.astype(sorted_prob.dtype))[:, None]

        y_flat = mx.zeros((n_tokens, self.dim), dtype=gathered.dtype)
        y_flat = y_flat.at[sorted_token_idx].add(gathered)
        return (
