...
def accumulate_flat_grads(
    ...
    flat_items = tree_flatten(grads_tree)
    if accum is None:
        return {k: g * scale for k, g in flat_items}
    for k, g in flat_items:
...
_FAST_RMS_NORM_NONE_OK = True


def rms_norm(x: mx.array, eps: float = 1e-6) -> mx.array:
    global _FAST_RMS_NORM_NONE_OK
    if _FAST_RMS_NORM_NONE_OK:
        try:
            return mx.fast.rms_norm(x, None, eps).astype(x.dtype)
        except (AttributeError, TypeError, ValueError):
            _FAST_RMS_NORM_NONE_OK = False
...
class DepthMixer(nn.Module):
    ...
    def __call__(
        ...
        scores = mx.stack(
            [mx.sum(key * query, axis=-1) * self.scale for key in keys],
            axis=-1,
        ).astype(mx.float32)
        ...
        weights = mx.softmax(scores, axis=-1, precise=True).astype(values[0].dtype)
...
class Block(nn.Module):
    def __init__(
        ...
        if attnres_enabled:
            self.attn_mixer = DepthMixer(dim, attnres_max_sources, attnres_num_loops, attnres_loop_specific, attnres_use_gate)
            self.mlp_mixer = DepthMixer(dim, attnres_max_sources, attnres_num_loops, attnres_loop_specific, attnres_use_gate)
    ...
    def full_attnres_forward(
        ...
        # Full AttnRes mode: each residualized sublayer issues its own
        # pseudo-query over the complete prior source history. The history starts
        # with the token embedding, then appends the raw output of every attention
        # and MLP sublayer in unrolled depth order. AttnResHistory caches keys.
        attn_in = self.attn_mixer(history, loop_idx=loop_idx)
        attn_out = self.attn_scale.astype(attn_in.dtype)[None, None, :] * self.attn(self.attn_norm(attn_in))
        history.append(attn_out)

        mlp_in = self.mlp_mixer(history, loop_idx=loop_idx)
        mlp_out = self.mlp_scale.astype(mlp_in.dtype)[None, None, :] * self.mlp(self.mlp_norm(mlp_in))
...
class GPT(nn.Module):
    ...
    def __init__(
        ...
        if self.attnres_enabled:
            self.final_mixer = DepthMixer(dim, args.attnres_max_sources, 1, False, False)
    ...
    def _stable_loop_input(self, h: mx.array, e: mx.array) -> mx.array:
        ...
        inject = self.prelude_inject_scale.astype(h.dtype).reshape(1, 1, -1)
    ...
    def __call__(self, input_ids: mx.array) -> mx.array:
        ...
        # Full Attention Residuals. The history is global across the entire
        # unrolled computation, not reset at prelude/core/coda boundaries. Each
        # attention and MLP sublayer appends its raw output to this cache, so later
        # sublayers can attend over every previous source plus the embedding.
        ...
        # Use a cheap deterministic readout for recurrence seeding. This avoids
        # making the final learned output query responsible for intermediate
        # stage state, while still deriving the seed from the full source history.
...
def main() -> None:
    ...
    log(
        ...
        f"attnres:{args.attnres_enabled} scope:full_all_sublayers "
