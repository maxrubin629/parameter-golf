# Record: LoopFullAttnRes + LoopQ + XSA

**val_bpb: 1.18871** (3-seed mean, std 0.00011, post-int8) | **3.07055 nats** | **~14.26 MB** | 8xH100 SXM, 600s | No TTT

This is a non-SOTA 10-minute / 16MB submission exploring recurrent weight tying with learned routing over loop/depth residual history. The model uses a prelude-core-coda layout, a recurrent middle section, full attention residual mixing, loop-specific learned depth queries, and exclusive self-attention in the recurrent core.
...
|------|------:|--------:|--------------:|------------------:|-------------------:|--------------:|--------------:|
| 42   | 5,507 | 108.96 | 1.1832 | **1.18875284** | 3.07067205 | 14,179,856 | 14,256,636 |
| 1337 | 5,510 | 108.90 | 1.1830 | **1.18878580** | 3.07075719 | 14,182,414 | 14,259,194 |
| 2025 | 5,511 | 108.88 | 1.1829 | **1.18857907** | 3.07022317 | 14,176,192 | 14,252,972 |
| **Mean** | **5,509** | **108.91** | **1.18303** | **1.18870590** | **3.07055080** | **14,179,487** | **14,256,267** |
...
The effective depth is therefore nine block applications, but the middle computation is weight-tied through recurrence.
...
- **Artifact <= 16,000,000 bytes decimal**: max counted bytes here are 14,259,194 (`train_gpt.py` bytes + compressed int8 model bytes).
...
- **Tokenizer/data**: canonical SP8192 FineWeb data and tokenizer; no tokenizer or dataset changes.
- **Self-contained code**: the model, optimizer, validation, and int8 zlib serialization path are in `train_gpt.py`.
...
for SEED in 42 1337 2025; do
  ...
  ARCH_NAME=loop_fullattnres_loopq_xsa \
  SEED=$SEED \
