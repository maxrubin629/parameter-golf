...
class Hyperparameters:
    ...
    train_batch_tokens_late: int = int(os.environ.get("TRAIN_BATCH_TOKENS_LATE", "0"))
    train_batch_tokens_switch_frac: float = float(os.environ.get("TRAIN_BATCH_TOKENS_SWITCH_FRAC", "1.0"))
    ...
    def _build_extra_schedule(self, num_layers: int, raw: str) -> list[int]:
        ...
        if raw:
            ...
            if not schedule or max(schedule) >= num_layers or min(schedule) < 0:
                raise ValueError(f"Invalid EXTRA_LOOP_SCHEDULE={raw!r} for NUM_LAYERS=9")
            return schedule
        if num_layers >= 7:
            return [4, 5, 6]
...
def ddp_grad_accum_steps(args: Hyperparameters, world_size: int) -> int:
    ...
    if args.train_batch_tokens_late > 0:
        local_tokens_late = args.train_batch_tokens_late // (world_size * grad_accum_steps)
        if local_tokens_late % args.train_seq_len != 0:
            raise ValueError(
                f"TRAIN_BATCH_TOKENS_LATE={args.train_batch_tokens_late} gives per-rank microbatch {local_tokens_late}, "
                f"not divisible by TRAIN_SEQ_LEN={args.train_seq_len}."
            )
    return grad_accum_steps
def scheduled_train_batch_tokens(args: Hyperparameters, step: int, elapsed_ms: float, max_wallclock_ms: float | None) -> int:
    late = args.train_batch_tokens_late
    if late <= 0 or late == args.train_batch_tokens:
        return args.train_batch_tokens
    frac = min(max(args.train_batch_tokens_switch_frac, 0.0), 1.0)
    if max_wallclock_ms is not None and max_wallclock_ms > 0:
        return late if elapsed_ms >= frac * max_wallclock_ms else args.train_batch_tokens
    return late if step >= int(frac * args.iterations) else args.train_batch_tokens
...
def main() -> None:
    ...
    log0(
        ...
        f"train_batch_tokens_switch_frac:{args.train_batch_tokens_switch_frac:.3f} "
    ...
    step = 0
    active_train_batch_tokens = args.train_batch_tokens
    while True:
        ...
        train_batch_tokens = scheduled_train_batch_tokens(args, step, elapsed_ms, max_wallclock_ms)
        if train_batch_tokens != active_train_batch_tokens:
            active_train_batch_tokens = train_batch_tokens
            log0(
                f"train_batch_tokens_switch step:{step} elapsed_ms:{elapsed_ms:.0f} "
                f"train_batch_tokens:{active_train_batch_tokens}"
            )
        ...
        for micro_step in range(grad_accum_steps):
            if distributed:
                model.require_backward_grad_sync = micro_step == grad_accum_steps - 1
            x, y = train_loader.next_batch(train_batch_tokens, args.train_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = model(x, y)
