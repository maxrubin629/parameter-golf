...
class CausalSelfAttention(nn.Module):
    ...
    def __call__(self, x: mx.array) -> mx.array:
        ...
        if self.xsa_enabled:
            # Group query heads by their shared GQA value head, then subtract the
            # projection of each head's attention output onto the token's own value
            # vector without materializing a repeated value tensor.
            rep = self.num_heads // self.num_kv_heads
            y_grouped = y.reshape(bsz, self.num_kv_heads, rep, seqlen, self.head_dim)
            v_self = v.astype(y.dtype)[:, :, None, :, :]
            v_self = v_self * mx.rsqrt(mx.sum(v_self * v_self, axis=-1, keepdims=True) + 1e-6)
            proj = mx.sum(y_grouped * v_self, axis=-1, keepdims=True) * v_self
            if self.xsa_learned:
                lam = self.xsa_lambda.astype(y.dtype).reshape(self.num_kv_heads, rep)[None, :, :, None, None]
...
class Block(nn.Module):
    def __init__(
        ...
        self.resid_mix = mx.stack((mx.ones((dim,), dtype=mx.float32), mx.zeros((dim,), dtype=mx.float32)))
...
class GPT(nn.Module):
    ...
    def loss(self, input_ids: mx.array, target_ids: mx.array) -> mx.array:
        ...
        embed_weight = self.tok_emb.weight.astype(x.dtype)
        if self.logit_chunk_tokens <= 0 or x.shape[0] <= self.logit_chunk_tokens:
            logits_proj = x @ embed_weight.T
        ...
        for s in range(0, n, self.logit_chunk_tokens):
            ...
            logits_proj = x[s:e] @ embed_weight.T
            logits = self.softcap(logits_proj)
