...
ARCH_NAME = "sp8192_loop_xsa_attnres_loopq"
ARCH_LOOPED = True
...
class Hyperparameters:
    ...
    attnres_max_sources: int = int(os.environ.get("ATTNRES_MAX_SOURCES", 6))
...
class DepthMixer(nn.Module):
    # Tiny Block-AttnRes-style mixer over same-position block/source states.
    ...
    def __call__(self, sources: list[mx.array], loop_idx: int) -> mx.array:
        ...
        weights = mx.softmax(scores, axis=-1, precise=True).astype(stacked.dtype)
...
class GPT(nn.Module):
    ...
    # - optional Block-AttnRes mixers replace additive residual routing in every stage
    # - recurrent-core Block-AttnRes mixers get separate learned slots per loop iteration
    def __init__(
        self,
        vocab_size: int,
        num_layers: int,
    ...
    def __call__(self, input_ids: mx.array) -> mx.array:
        ...
        # Prelude. If Attention Residuals are enabled, they replace additive
        # residual routing here as well as in the recurrent core and coda.
        prelude_states: list[mx.array] = [x]
        ...
        e = rms_norm(x)
        h = e
        prev_loop_blocks: list[mx.array] = []

        # Recurrent core. The expensive block weights are shared across loops, but
        # every loop iteration receives its own tiny AttnRes query/bias/gate slot.
        core_start = self.prelude_layers
        for loop_idx in range(self.num_loops):
            if self.stable_recurrence:
                h = self._stable_loop_input(h, e)
            loop_start = h
            current_loop_blocks: list[mx.array] = []
            core_states: list[mx.array] = [loop_start]
            base_sources: list[mx.array] = [e]
        ...
        # Coda. Attention Residuals remain active here too, with a single mixer slot
        # because coda layers run once rather than once per recurrent iteration.
        coda_start = self.prelude_layers + self.core_layers
        coda_states: list[mx.array] = [h]
    ...
    def loss(self, input_ids: mx.array, target_ids: mx.array) -> mx.array:
        ...
        lm_weight = self.tok_emb.weight.astype(x.dtype).T
        if self.logit_chunk_tokens <= 0 or x.shape[0] <= self.logit_chunk_tokens:
            logits_proj = x @ lm_weight
...
class Muon:
    ...
    def __init__(self, keys: list[str], params: dict[str, mx.array], args: Hyperparameters):
        ...
        self.scales = {
            k: math.sqrt(max(1.0, float(params[k].shape[0]) / float(params[k].shape[1])))
            for k in keys
        }
...
def main() -> None:
    ...
    compiled_loss = mx.compile(loss_fn, inputs=model.state, outputs=model.state)
    ...
    log(
        ...
        f"attnres:{args.attnres_enabled} scope:all_blocks "
        f"attnres_core_loop_slots:{args.attnres_enabled} "
        f"prev_loop_blocks:{args.attnres_prev_loop_blocks} max_sources:{args.attnres_max_sources} "
