...
# HYPERPARAMETERS
# ==============================================================================
# Current-record architecture port:
# - SP8192, 11 physical transformer blocks, width 512
# - layers 3-5 are reused after ENABLE_LOOPING_AT, giving 17 virtual block applications
# - 8 attention heads with 4 KV heads (GQA), QK gain 5.25, partial RoPE
# - 4x LeakyReLU^2 MLP, parallel residuals from layer 7, sigmoid-gated skips
class Hyperparameters:
    ...
    # Model (defaults match the current record architecture).
    ...
    def training_frac(self, step: int, elapsed_ms: float) -> float:
        if self.max_wallclock_seconds <= 0:
            return step / max(self.iterations, 1)
        return elapsed_ms / max(1000.0 * self.max_wallclock_seconds, 1e-9)
...
class MLP(nn.Module):
    # Current record uses LeakyReLU(0.5)^2 with a 4x expansion.
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init, rope_dims, xsa_enabled)
...
class GPT(nn.Module):
    # Current-record topology:
    # - 11 physical blocks with layers 3-5 optionally looped into a 17-application schedule
    # - encoder/decoder split over that virtual schedule, with sigmoid-gated skip blending
    ...
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: float,
                 ...
                 qk_gain_init: float, rope_dims: int, ln_scale: bool, xsa_last_n: int, num_loops: int,
                 loop_start: int, loop_end: int, enable_looping_at: float, parallel_residual_start: int,
        ...
        self.looping_active = self.num_loops > 0 and enable_looping_at <= 0.0
        self.blocks = [
            Block(
                dim,
                num_heads,
                ...
                layer_idx=i,
                ln_scale=ln_scale,
                xsa_enabled=xsa_last_n > 0 and i >= max(0, num_layers - xsa_last_n),
            )
        ...
        for skip_idx, i in enumerate(dec_iter):
            if skip_idx < self.num_skip_weights and skips:
                ...
                if self.skip_gates is not None:
                    gate = 1.0 / (1.0 + mx.exp(-self.skip_gates[skip_idx].astype(x.dtype)))[None, None, :]
                    x = scaled_skip + gate * (x - scaled_skip)
                else:
                    x = x + scaled_skip
            x = self.blocks[i](x, x0)
...
class SplitOptimizers:
    ...
    def __init__(self, model: GPT, args: Hyperparameters):
        ...
        self.scalar_keys = [
            k
            for k, p in params.items()
            if k in {"skip_weights", "skip_gates"} or (k.startswith("blocks.") and (p.ndim < 2 or any(pattern in k for pattern in CONTROL_TENSOR_NAME_PATTERNS)))
        ]
...
def main() -> None:
    ...
    def recompile_model_fns() -> None:
        nonlocal compiled_loss, compiled_loss_and_grad
        compiled_loss = mx.compile(lambda x, y: model.loss(x, y), inputs=model.state, outputs=model.state)
        compiled_loss_and_grad = mx.compile(
            nn.value_and_grad(model, lambda x, y: model.loss(x, y)),
            inputs=model.state,
    ...
    log(
        f"architecture:current_record rope_dims:{args.rope_dims} qk_gain:{args.qk_gain_init} "
        f"mlp_mult:{args.mlp_mult} ln_scale:{args.ln_scale} xsa_last_n:{args.xsa_last_n} "
        f"parallel_residual_start:{args.parallel_residual_start} skip_gates:{args.skip_gates_enabled}"
    )
    log(
        f"recurrence:num_loops:{args.num_loops} loop_start:{args.loop_start} loop_end:{args.loop_end} "
        f"enable_looping_at:{args.enable_looping_at} looping_active:{model.looping_active} "
        f"encoder:{model.encoder_indices} decoder:{model.decoder_indices}"
    )
    log(
    ...
    log(
        ...
        f"skip_gates:{model.skip_gates.dtype if model.skip_gates is not None else 'disabled'}"
    )
    ...
    while True:
        ...
        training_frac = args.training_frac(step, train_time_ms + 1000.0 * (time.perf_counter() - t0))
        if args.num_loops > 0 and not model.looping_active and training_frac >= args.enable_looping_at:
            model.looping_active = True
            recompile_model_fns()
            log(
                f"layer_loop:enabled step:{step} frac:{training_frac:.3f} "
                f"encoder:{model.encoder_indices} decoder:{model.decoder_indices}"
            )
