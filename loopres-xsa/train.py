...
DEFAULT_TRAIN_BATCH_TOKENS_LATE = 786432
DEFAULT_TRAIN_BATCH_TOKENS_SWITCH_FRAC = 0.55
...
class Hyperparameters:
    ...
    train_batch_tokens_late: int = int(os.environ.get("TRAIN_BATCH_TOKENS_LATE", DEFAULT_TRAIN_BATCH_TOKENS_LATE))
    train_batch_tokens_switch_frac: float = float(os.environ.get("TRAIN_BATCH_TOKENS_SWITCH_FRAC", DEFAULT_TRAIN_BATCH_TOKENS_SWITCH_FRAC))
    ...
    def train_batch_tokens_for(self, step: int, elapsed_ms: float) -> int:
        switch_frac = min(max(self.train_batch_tokens_switch_frac, 0.0), 1.0)
        if self.max_wallclock_seconds > 0:
            progress = elapsed_ms / max(1000.0 * self.max_wallclock_seconds, 1e-9)
        else:
            progress = step / max(self.iterations, 1)
        return self.train_batch_tokens_late if progress >= switch_frac else self.train_batch_tokens
...
def ddp_grad_accum_steps(args: Hyperparameters, world_size: int) -> int:
    ...
    for name, global_tokens in (
        ("TRAIN_BATCH_TOKENS", args.train_batch_tokens),
        ("TRAIN_BATCH_TOKENS_LATE", args.train_batch_tokens_late),
    ):
        local_tokens = global_tokens // (world_size * grad_accum_steps)
        if local_tokens % args.train_seq_len != 0:
            raise ValueError(
                f"{name}={global_tokens} gives per-rank microbatch {local_tokens}, "
...
def main() -> None:
    ...
    log0(
        ...
        f"train_batch_tokens_late:{args.train_batch_tokens_late} train_batch_tokens_switch_frac:{args.train_batch_tokens_switch_frac:.3f} "
    ...
    while True:
        ...
        train_batch_tokens = args.train_batch_tokens_for(step, elapsed_ms)
        zero_grad_all()
